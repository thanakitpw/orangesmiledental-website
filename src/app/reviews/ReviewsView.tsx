...
import type { CaseItem, GalleryData } from '@/lib/site-content';
...
/** 'all', or a category key. Not a closed union any more: treatment categories
 *  are rows now, and the clinic can add one without a deploy. */
type Filter = string;
type GalFilter = string;

/** How many gallery tiles a "show more" press adds. */
const GAL_PAGE = 12;

export interface ReviewsViewData {
  cases: CaseItem[];
  gallery: GalleryData;
}

export function ReviewsView({ cases, gallery }: ReviewsViewData) {
  const SITE = useSiteSettings();
  const { cats: galleryCats, items: galleryItems } = gallery;
  ...
  const galAll = galCat === 'all' ? galleryItems : galleryItems.filter((g) => g.cat === galCat);
  ...
  // Built from the cases that exist rather than a hardcoded list of four, so a
  // new treatment category gets its chip automatically and a category with no
  // cases left does not leave a chip that filters down to nothing.
  const caseCats = new Map(cases.map((c) => [c.cat, tl(c.catLabel)]));
  const chipDefs: { key: Filter; label: string }[] = [
    { key: 'all', label: t('ทั้งหมด', 'All') },
    ...[...caseCats].map(([key, label]) => ({ key, label })),
  ];
  ...
  // getGallery() has already dropped categories that hold no image.
  const galChips: { key: GalFilter; label: string }[] = [
    { key: 'all', label: t('ทั้งหมด', 'All') },
    ...galleryCats.map((c) => ({ key: c.key, label: tl(c.label) })),
  ];

  const galCatLabel = (key: string) => {
    const cat = galleryCats.find((c) => c.key === key);
    return cat ? tl(cat.label) : key;
  };
      ...
      >
        ...
        >
          ...
          <div style={{ columns: '4 250px', columnGap: 20 }}>
            {galShown.map((g, i) => {
              const catLabel = galCatLabel(g.cat);
              return (
                <div
                  key={g.img}
                  className="case-card"
        ...
        {lightbox !== null && galAll[lightbox] && (
          ...
          >
            ...
            <img
              ...
              alt={galCatLabel(galAll[lightbox].cat)}
